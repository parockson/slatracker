...
if uploaded_sales and sla_data:
    ...
    with c2:
        deb_col = st.selectbox(
            "Debit Amt (for Cor)", df_raw.columns, 
            index=get_best_match(['debit', 'dr'], df_raw.columns),
            help="Look for 'Debit Amt' or just 'Debit'"
        )
        cre_col = st.selectbox(
            "Credit Amt (for Self/Asst)", df_raw.columns, 
    ...
    with c3:
        mar_col = st.selectbox(
            ...
            index=get_best_match(['gr. rev', 'gross', 'margin', 'rev'], df_raw.columns),
